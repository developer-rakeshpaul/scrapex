...
console.log(result.normalizedBlocks); // Debug: see classified blocks
```
...
- Use `mode: 'summary'` when you want the most relevant blocks by score (useful for short previews).
- Set `maxChars` with `truncate: 'sentence'` to keep readable summaries.
- Use `dropSelectors` to remove large non-content elements before parsing (nav, footer, ads, or SVGs).
- Pass `languageHint` when you already know the page language to improve metadata.
- Use `debug: true` to inspect `normalizedBlocks` and tune custom classifiers.
